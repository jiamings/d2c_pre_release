...
class AutoEncoder(nn.Module):
    def __init__(self, args, writer, arch_instance):
        ...
        self.enc_sampler = Conv2D(int(self.num_channels_enc * mult), self.num_latent_per_group, kernel_size=3, padding=1, bias=True)

        self.dec0 = Conv2D(self.num_latent_per_group, int(self.num_channels_dec * mult),
    ...
    def init_normal_sampler(self, mult):
        ...
        for s in range(self.num_latent_scales):
            for g in range(self.groups_per_scale[self.num_latent_scales - s - 1]):
                ...
                num_c = int(self.num_channels_enc * mult)
                cell = Conv2D(num_c, self.num_latent_per_group, kernel_size=3, padding=1, bias=True)
    ...
    def forward(self, x, get_latent=False, from_latent=False, with_latent=False, get_half=False, reshape=True, detach=False):
        if not from_latent:
            s = self.stem(2 * x - 1.0)
            ...
            latent = self.enc_sampler(ftr)
            
            if get_half:
                latent_return = latent.view((latent.size(0), -1))
                s = latent_return.size(1)
                latent_return = latent_return[:,:s//2]
                latent_return = self.fc(latent_return)
            else:
                if reshape:
                    latent_return = self.fc(latent.view((latent.size(0), -1)))
                else:
                    latent_return = latent
            if get_latent:
                return latent_return
        else:
            latent = x
        ...
        # print("the size of latent is ", s.size())
        s = self.dec0(s)
        idx_dec = 0
        batch_size = s.size(0)
    ...
    def decoder_output(self, logits):
        ...
        elif self.dataset in {'cifar10', 'cifar100', 'celeba_64', 'celeba_256', 'imagenet_32', 'imagenet_64', 'ffhq','ffhq_32',
                              'lsun_bedroom_128', 'lsun_bedroom_256', 'mnist_new', 'satellite'}:
